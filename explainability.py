...
def load_model_and_data_for_shap():
    try:
        ...
        # Load a small sample of the training data for SHAP
        data = pd.read_csv('path/to/your/train_data.csv')
        sample_data = data.sample(n=100, random_state=42).copy()
        
        # Ensure the sample data has the same features as your model
        # You'll need to re-run your feature engineering on this sample
        ...
        mean_map_intensity = {
            'small': 0.5, 'moderate': 1.2, 'heavy': 2.5
        }
        mean_map_community = {
            '1': 0.8, '2': 1.5, '3': 2.0
        }
        ...
        sample_data = sample_data.drop(columns=['community_code', 'district_code'])
        sample_data = sample_data.drop(columns=['Target', 'ID', 'user_id'])
...
st.markdown("""
SHAP (SHapley Additive exPlanations) helps us understand how each feature contributed to the model's predictions.

- **Summary Plot**: Shows which features are most important overall and their impact.
- **Dependency Plot**: Reveals how a single feature's value affects the prediction.
""")
...
if model and not sample_data.empty:
    ...
    st.info("This plot shows the feature importance for all predictions. The position of each point on the x-axis represents the impact of that feature on a specific prediction.")
    fig, ax = plt.subplots(1, 1, figsize=(10, 8))
    # For multiclass, SHAP values are a list of arrays. Use the first class (e.g., 'heavy') for the summary plot.
    ...
    # Let the user select a feature to visualize
